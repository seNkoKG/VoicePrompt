...
internal sealed class PerformanceSnapshot
{
    ...
    public double? MedianRealtimeSpeed => NullablePercentile(
        Samples.Where(sample => sample.RecordingSeconds > 0 && sample.TotalSeconds > 0)
            .Select(sample => sample.RecordingSeconds!.Value / sample.TotalSeconds),
        0.50);
    ...
    internal static PerformanceSnapshot Parse(IEnumerable<string> lines, int maxSamples = 50)
    {
        ...
        foreach (string line in lines)
        {
            ...
            if (latency.Success)
            {
                ...
                pending.PrimarySeconds = ParseDouble(latency.Groups["primary"].Value);
                pending.RetrySeconds = ParseDouble(latency.Groups["retry"].Value);
                pending.TotalSeconds = ParseDouble(latency.Groups["total"].Value);
                pending.HasLatency = true;
            }

            Match language = LanguagePattern.Match(line);
            if (!language.Success || !pending.HasLatency)
                continue;
            ...
            samples.Add(new PerformanceSample(
                ...
                language.Groups["language"].Value,
                ParseDouble(language.Groups["confidence"].Value),
                ParseInt(language.Groups["segments"].Value)));
